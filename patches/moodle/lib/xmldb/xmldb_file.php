 ...
 *
 * Local playground patch:
 * Bypass xmlize()/ext-xml callbacks for XMLDB loading. php-wasm currently
 * crashes inside xml_parse callback dispatch, so we build the XMLDB array
 * structure directly from DOMDocument instead.
 *
 * @package    core_xmldb
...
class xmldb_file extends xmldb_object {
    ...
    /**
     * Convert a DOM element into the subset of the xmlize() structure used by XMLDB.
     *
     * @param DOMElement $element
     * @return array
     */
    protected function dom_element_to_xmlarr(DOMElement $element) {
        $result = array(
            '@' => array(),
            '#' => array(),
        );
        ...
        foreach ($element->childNodes as $child) {
            if ($child instanceof DOMElement) {
                if (!isset($result['#'][$child->tagName])) {
                    $result['#'][$child->tagName] = array();
                }
                $result['#'][$child->tagName][] = $this->dom_element_to_xmlarr($child);
            } else if ($child instanceof DOMText || $child instanceof DOMCdataSection) {
                $text = $child->nodeValue;
                if ($text === '0' || trim($text) !== '') {
                    if (!isset($result[0])) {
                        $result[0] = $text;
                    } else if (is_string($result[0])) {
                        $result[0] .= $text;
                    }
                }
            }
    ...
    /**
     * Build the XMLDB structure array without using xmlize()/xml_parse callbacks.
     *
     * @param string $contents
     * @return array|false
     */
    protected function dom_to_xmldb_array($contents) {
        $dom = new DOMDocument();
        ...
        return array(
            $dom->documentElement->tagName => $this->dom_element_to_xmlarr($dom->documentElement),
        );
    }
    ...
    public function loadXMLStructure() {
        if ($this->fileExists()) {
            ...
            $xmlarr = $this->dom_to_xmldb_array($contents);
            if ($xmlarr === false) {
                return false;
            }
