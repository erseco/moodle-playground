...
export async function fetchWithProgress(url, onProgress = () => {}) {
  ...
  const usePreallocatedBuffer = Number.isFinite(total) && total > 0;
  const preallocated = usePreallocatedBuffer ? new Uint8Array(total) : null;
  const chunks = usePreallocatedBuffer ? null : [];
  let loaded = 0;
  ...
  if (preallocated) {
    ...
    return loaded === preallocated.byteLength ? preallocated : preallocated.slice(0, loaded);
  }
